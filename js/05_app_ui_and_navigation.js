...
function subG(nxt){
  ...
  r.answers.push({cardId:card.id,prev,next:nxt,ms});
    if(r.index<r.queue.length-1){r.index++;r.flipped=!1;r.cardStart=Date.now();debouncedSave();renRev()}else{r.end=Date.now();debouncedSave();if(typeof FireSync!=='undefined'&&FireSync.isConnected)FireSync.pushToCloud();goRecap(!1)}
}
