...
const normaliserReponse = (texte) => (texte || '')
.toLowerCase().normalize('NFD')
.replace(DIACRITIQUES_REGEX, '')
.replace(/[_-]/g, ' ')
.replace(/[^a-z' ]+/g, ' ')
.replace(/\s+/g, ' ')
.trim();
...
function appliquerThemeCarte() {
const el = elementsDOM.conteneurFlashcard;
if (!el) return;
el.classList.remove('light-mode-card-neutral', 'dark-mode-card-neutral');
el.classList.add(etatApplication.modeSombre ? 'dark-mode-card-neutral' : 'light-mode-card-neutral');
}
